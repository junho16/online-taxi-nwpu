...
public class GpsUtil {
     ...
     * @return [纬度, 经度]
     */
    public static TracePos calGCJ02toWGS84(TracePos source) {
        double longitude = source.getLon();
        double latitude = source.getLat();
        Point dev = calDev(latitude, longitude);
        double retLat = latitude - dev.getLatitude();
        double retLon = longitude - dev.getLongitude();
        ...
        return new TracePos( retLon ,retLat , source.getSpeed());
    }
...
//    //输入GCJ经纬度 转WGS纬度
//    public static TracePos GcjToWsg (TracePos origin) {
...
//        double sqrtMagic = Math.sqrt(magic);
//        dLat = (dLat * 180.0) / ((a * (1 - ee)) / (magic * sqrtMagic) * PI);
//        return new TracePos((lon - dLon) ,( lat - dLat ) , origin.getSpeed());
//    }
//
//
//    //输入GCJ经纬度 转WGS纬度
//    public static double WGSLat (double lon,double lat) {
//        double PI = 3.14159265358979324;//圆周率
//        double a = 6378245.0;//克拉索夫斯基椭球参数长半轴a
//        double ee = 0.00669342162296594323;//克拉索夫斯基椭球参数第一偏心率平方
//        double dLat = transformLat(lon - 105.0, lat - 35.0);
...
//
//    //输入GCJ经纬度 转WGS经度
//    public static double WGSLon (double lon,double lat) {
//        double PI = 3.14159265358979324;//圆周率
//        double a = 6378245.0;//克拉索夫斯基椭球参数长半轴a
//        double ee = 0.00669342162296594323;//克拉索夫斯基椭球参数第一偏心率平方
//        double dLon = transformLon(lon - 105.0, lat - 35.0);
