package com.nwpu.myonlinetaxi.algorithm;
...
/**
 * 路口车辆算法
 * 根据 车辆距离设备(路口)的距离 车辆速度 信号灯的状态 剩余时间 给出行车建议
 */
public class Guidance {

    private static final Logger logger = LoggerFactory.getLogger(Guidance.class);

    static final double  Speed_limit = 60;            // 最大车速 m/s        1
...
//    static final int GREEN = 1;
    ...
    /**
     *
     * @param S_ego 车辆距信号灯距离
     * @param V_ego 车辆速度
     * @param tag 信号灯状态
     * @param tr 剩余时间
     * @return
     */
    public static String getSuggest(int S_ego , int V_ego , int tag , int tr)
    {
        logger.info("S_ego:" + S_ego + " " + "V_ego:" + V_ego + " " + "tag:" + tag + " " +"tr:" + tr);
        //////////////////////////////
        //输入
        Guidance.S_ego = S_ego;
        Guidance.V_ego = V_ego;
        Guidance.tag = tag;
        Guidance.tr = tr;
//        Guidance.S_ego = 120;
//        Guidance.V_ego = 10;
