 ...
 */
public class PassengerOpenApiExtention {
    /**
     * 物模型上报格式公共字段-id
     */
    ...
    public PassengerOpenApiExtention(OpenApi openApi) {
     ...
     * 单个属性功能点上报
     *
     *  @param passengerList 标识符为passengerList的属性功能点的值
     *  @param timeout 超时时间，单位为毫秒
     */
    public int passengerListPropertyUpload(PassengerListStructDTO[] passengerList, long timeout)
            throws Exception {
        return this.propertyUpload(timeout, passengerList);
    }
     ...
     *  @param passengerList 标识符为passengerList的属性功能点的值
     */
    public int propertyUpload(long timeout, PassengerListStructDTO[] passengerList) throws
            Exception {
        ...
        if (!Objects.isNull(JSON.toJSON(passengerList))) {
                Map<String, Object> val1 = new HashMap<>();
                val1.put("value", JSON.toJSON(passengerList));
                val1.put("time", now);
                params.put("passengerList", val1);
        }
